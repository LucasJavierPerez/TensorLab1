...
function SideNav({ collapsed, setCollapsed }: { collapsed: boolean; setCollapsed: (b: boolean) => void }) {
  const groups = [
    {
      label: "OPERACIÓN",
      items: [
        { icon: LayoutDashboard, label: "Panel", active: true },
        { icon: Building2, label: "Empresas", badge: "12" },
    ...
    },
    {
      ...
      items: [
        ...
        { icon: Cpu, label: "Modelos", badge: "42" },
        { icon: Activity, label: "Telemetría" },
  ...
  return (
    ...
    >
      <div className="h-14 border-b border-sidebar-border flex items-center px-4 gap-2">
        {collapsed ? (
          <Link to="/"><Logo className="[&>span]:hidden" /></Link>
        ) : (
          <Link to="/"><Logo /></Link>
        )}
        <button
          ...
          <div key={g.label}>
            ...
            <ul className="space-y-0.5">
              {g.items.map((it) => (
                <li key={it.label}>
                  <button
                    className={`w-full flex items-center gap-2.5 h-9 px-2 rounded-md text-[13px] transition group
                      ${it.active ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/80 hover:bg-sidebar-accent/60 hover:text-sidebar-foreground"}`}
                  >
                    <it.icon className={`h-4 w-4 ${it.active ? "text-glow" : ""}`} />
                    ...
                    {it.active && <span className="absolute left-0 w-[2px] h-5 bg-glow rounded-r" style={{ position: "relative" }} />}
                  </button>
                </li>
              ))}
            </ul>
